...
pub struct IcebergTableProvider {
    ...
    /// construction time and when pinning a snapshot; scans/writes always reload
    /// fresh metadata from the catalog.
    table: Table,
    /// A reference-counted arrow `Schema` (cached at construction, recomputed when
    /// a snapshot is pinned so it always matches the schema being read)
...
impl IcebergTableProvider {
    ...
    /// actually being read even when the table's schema has since evolved. The
    /// snapshot id is threaded into the scan node, so it is serialized and honored
    /// by a distributed engine as well.
